...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        <h1 class="text-3xl font-bold text-blue-600 mb-8">Réception de Sacs</h1>
        ...
        <form action="{{ route('bags.store-received') }}" method="POST" class="bg-white rounded-lg shadow-lg p-6 mb-8">
            @csrf
            <div class="mb-4">
                ...
                <select name="bag_id" id="bag_id" required
                    ...
                    @foreach($bags as $bag)
                        <option value="{{ $bag->id }}">{{ $bag->name }}-{{ round($bag->price,1) }} XAF</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-lg overflow-hidden">
            <h2 class="text-xl font-bold p-4 bg-blue-500 text-white">Historique des Réceptions</h2>
            <table class="min-w-full">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($bags->flatMap->transactions->where('type', 'received')->sortByDesc('transaction_date') as $transaction)
                        <tr>
