...
@section('content')
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Ajouter un nouvel ingrédient') }}
        </h2>
    </x-slot>
                    ...
                    <form action="{{ route('recipe.ingredients.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="unit" id="unit" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                ...
                                <option value="ml" {{ old('unit') == 'ml' ? 'selected' : '' }}>Millilitre (ml)</option>
                                <option value="l" {{ old('unit') == 'l' ? 'selected' : '' }}>Litre (l)</option>
                                <option value="cs" {{ old('unit') == 'cs' ? 'selected' : '' }}>Cuillère à soupe</option>
                                <option value="cc" {{ old('unit') == 'cc' ? 'selected' : '' }}>Cuillère à café</option>
                                <option value="pièce" {{ old('unit') == 'pièce' ? 'selected' : '' }}>Pièce</option>
                                <option value="pincée" {{ old('unit') == 'pincée' ? 'selected' : '' }}>Pincée</option>
                            </select>
